...
"if you tame me, then we shall need each other. To me, you will be unique in all the world. To you, I shall be unique in all the world if you tame me, it will be as if the sun came to shine on my life. I shall know the sound of a step that will be different from all the others. Other steps send me hurrying back underneath the ground. Yours will call me, like music, out of my burrow", "If you tame me, it would be as if the sun came to shine on my life.",
...
setquery = list(set(querysplit))

dt1 = datetime.now()
if("or" in setquery):
    setquery.remove("or")
    for i,quote in enumerate(data_list):
        words = re.sub(r'[^\w]', ' ', quote)
        words=words.split()
        for word in words:
            for item in setquery:
                if(item == word):
...
if("and" in setquery):
    ...
    for i,quote in enumerate(data_list):
        words = re.sub(r'[^\w]', ' ', quote)
        words=words.split()
        if((setquery[0] in words)and(setquery[1] in words)):
                print("Found at ", i,  data_list[i])
                print("--------------------------")


if(("and" in setquery) and ("or" not in setquery)) or (("and" in setquery) and ("or" in setquery)) or (("or" not in setquery) and ("and" not in setquery)):
    if "and" in setquery:
        setquery.remove("and")
    if "or" in setquery:
        setquery.remove("or")
